...
import { evaluateProfile } from '../../utils/aiService';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchAIScore = async () => {
      setLoading(true);
      try {
        const aiResult = await evaluateProfile(studentData);
        if (aiResult) {
          const breakdown = {
            skills: aiResult.skills,
            projects: aiResult.projects,
            extracurriculars: aiResult.extracurriculars,
            competitions: aiResult.competitions,
            achievements: aiResult.achievements,
            completeness: aiResult.completeness,
          };
          setAiScore({
            score: aiResult.totalScore,
            percentage: aiResult.totalScore,
          ...
          setAiScore(calculateAIScore(studentData));
        }
      } catch (error) {
  ...
  const statCards = [
    { label: 'Profile strength', val: loading ? '...' : `${aiScore?.percentage || 0}%`, icon: Target, c: 'var(--accent)' },
          ...
          >
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                ...
                {aiScore && !loading ? <ProfileRadar breakdown={aiScore.breakdown} size={180} /> : <div style={{ fontSize: '12px', color: 'var(--t3)' }}>Loading...</div>}
              </div>
              <div style={{
                display: 'grid',
                ...
                {aiScore && !loading && Object.entries(aiScore.breakdown).map(([key, data]) => (
                  <div key={key} style={{
                ...
                {loading && <div style={{ gridColumn: '1/-1', textAlign: 'center', color: 'var(--t3)', fontSize: '12px' }}>Loading breakdown...</div>}
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '14px' }}>
              <div style={{ background: 'var(--amber-dim)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 'var(--radius-md)', padding: '16px' }}>
                ...
                {loading ? (
                  <div style={{ fontSize: '12px', color: 'var(--t3)' }}>Loading AI insights...</div>
                ) : (
              ...
              <div style={{ background: 'var(--green-dim)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 'var(--radius-md)', padding: '16px' }}>
                ...
                {loading ? (
                  <div style={{ fontSize: '12px', color: 'var(--t3)' }}>Loading recommendations...</div>
                ) : (
